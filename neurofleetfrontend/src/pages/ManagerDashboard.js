...
const ManagerDashboard = () => {
  ...
  const getStatusStyle = (status) => {
    const map = {
      ...
      DRIVER_ACCEPTED: 'bg-indigo-500/20 text-indigo-400',
  ...
  const renderBookingCard = (booking, showActions) => (
      ...
      <div className="space-y-2 mb-4">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <div>
            ...
            <p className="text-white font-semibold">
              {booking.startTime ? new Date(booking.startTime).toLocaleString() : 'Not set'}
            </p>
          </div>
        </div>
      </div>
      ...
      {showActions && (
        selectedBooking?.id === booking.id ? (
          <div className="space-y-3">
            <div>
              <label className="block text-white/70 text-sm font-semibold mb-2">Assign Driver</label>
              <select
            ...
            <div className="flex gap-2">
              <button onClick={() => setSelectedBooking(null)} className="flex-1 btn-secondary">
                Cancel
              </button>
              <button onClick={handleApproveAndAssign} disabled={loading} className="flex-1 btn-primary">
                {loading ? 'Processing...' : '✅ Approve & Assign'}
              </button>
            </div>
          </div>
        ) : (
          <button onClick={() => setSelectedBooking(booking)} className="w-full btn-primary">
            Process Booking
          </button>
        )
      )}
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'allBookings':
        return (
          <div className="space-y-6">
            <h2 className="text-3xl font-bold text-white mb-4">📚 All Bookings ({allBookings.length})</h2>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6">
              <div className="glass-card p-6 text-center">
                ...
                <p className="text-4xl font-bold text-accent-green">{vehicles.filter((v) => v.status === 'AVAILABLE').length}</p>
              </div>
              <div className="glass-card p-6 text-center">
                <p className="text-white/60 text-sm mb-2">In Use</p>
                <p className="text-4xl font-bold text-accent-cyan">{vehicles.filter((v) => v.status === 'IN_USE').length}</p>
              </div>
              <div className="glass-card p-6 text-center">
                <p className="text-white/60 text-sm mb-2">Maintenance</p>
                <p className="text-4xl font-bold text-accent-pink">{vehicles.filter((v) => v.status === 'MAINTENANCE').length}</p>
              </div>
              <div className="glass-card p-6 text-center">
                        ...
                        <div>
                          ...
                          {aiPrediction && (
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              <span className="text-white/50 text-xs">Risk: {aiPrediction.prediction?.risk_score}%</span>
                            </div>
                          )}
                        </div>
                      </div>
            ...
            {maintenancePredictions.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="glass-card p-4 bg-red-500/10 border border-red-500/20">
                  ...
                  <p className="text-red-400 font-bold text-3xl">{maintenancePredictions.filter((p) => p.prediction?.priority === 'HIGH').length}</p>
                </div>
                <div className="glass-card p-4 bg-yellow-500/10 border border-yellow-500/20">
                  ...
                  <p className="text-yellow-400 font-bold text-3xl">{maintenancePredictions.filter((p) => p.prediction?.priority === 'MEDIUM').length}</p>
                </div>
                <div className="glass-card p-4 bg-blue-500/10 border border-blue-500/20">
                  ...
                  <p className="text-blue-400 font-bold text-3xl">{maintenancePredictions.filter((p) => p.prediction?.priority === 'LOW').length}</p>
                </div>
              </div>
            )}
                ...
                <div className="space-y-3">
                  {maintenancePredictions
                    .sort((a, b) => {
                      ...
                      return order[a.prediction?.priority] - order[b.prediction?.priority];
                    })
                    .map((item) => (
                      <div
                        key={item.vehicle_id}
                        className={`p-4 rounded-xl border-2 ${item.prediction?.priority === 'HIGH' ? 'bg-red-500/10 border-red-500/30' : item.prediction?.priority === 'MEDIUM' ? 'bg-yellow-500/10 border-yellow-500/30' : 'bg-blue-500/10 border-blue-500/30'}`}
                      >
                        <div className="flex items-start justify-between mb-3">
                          ...
                          <div className="text-right">
                            ...
                            <p className={`font-bold text-2xl ${item.prediction?.risk_score > 70 ? 'text-red-400' : item.prediction?.risk_score > 40 ? 'text-yellow-400' : 'text-blue-400'}`}>
      ...
      default:
        return (
          <div className="space-y-6">
            <h2 className="text-3xl font-bold text-white mb-4">📋 Manager Tasks ({pendingBookings.length})</h2>
          ...
          <div className="flex gap-2 pb-3 overflow-x-auto">
            {[
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg font-semibold transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-gradient-to-r from-accent-purple to-accent-pink text-white' : 'text-white/70 hover:text-white hover:bg-white/5'}`}
              >
                {tab.label}
              </button>
            ))}
